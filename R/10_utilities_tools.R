...
#
# Purpose: Provide miscellaneous utility functions
#   - Test object generators: test_polm(), test_stsp(), test_lmfd()
#   - Array manipulation: dbind(), array helpers
#   - Sequence helpers: iseq(), expand_letters()
#   - Matrix operations and data structure tools
#
# Related Files: All other files (supporting utilities)
# ===================================================================
...
#' 
#' @param x,y two vectors of length \eqn{p \leq q}{p \le q} respectively.
#'
#' @return The \eqn{p}-dimensional integer vector \code{j} (with unique elements).
#' 
#' @export
#' @keywords internal
#'
...
#' 
#' This simple tool may be used to create a random, \eqn{(m,n)}-dimensional, rational matrix in RMFD form 
#' \deqn{k(z) = d(z) c^{-1}(z)}{k(z) = d(z) c^{-1}(z)}
#' The degrees of the polynomials \eqn{(c'(z), d'(z))'} is denoted with \eqn{p} and \eqn{q} respectively.
#' 
#' We require \eqn{m>0} and \eqn{p\geq 0}{p\ge 0}. 
#' The right factor \eqn{c(z)} is normalized as \eqn{c(0)=I_n}{c(0)=I_n} where \eqn{I_n}{I_n} denotes the \eqn{(n,n)}-dimensional identity matrix.  
#' 
#' The user may prescribe lower bounds for the moduli of the zeroes and/or poles of the rational matrix.
#' In this case, the procedure simply generates (up to n.trials) random matrices until a matrix is found which satisfies the constraint. 
#' The standard deviation of the normal distribution, which is used to generate the random entries, is decreased in each step. 
#' Of course, this is a very crude method and it may fail or need a very large number of randomly generated matrices.
#'
#' @param dim integer vector \code{c(m,n)}. 
#' @param degrees integer vector \code{c(p,q)}.
#' @param digits integer, if non NULL then the randomly generated numbers are rounded to "digits" number of decimal places.
...
#'
#' @return \code{\link{rmfd}} object, which represents the generated rational matrix \eqn{k(z)} in RMFD form. 
#' @export
#' 
#' @examples 
...
#' 
#' @inheritParams test_polm
#' @param degree_max,degree_min Integer, vector, or matrix specifying minimal and maximal (column) degrees
#' @param col_start_matrix Force a matrix for lowest column degrees 
#'
...
#' @param nu    vector with the Kronecker indices (or \code{NULL}). Either the statespace dimension \code{s} 
#'              or the Kronecker indices \code{nu} must be non \code{NULL}. If both parameters are given, 
#'              then the parameter \code{s} is ignored. 
#' @param D     \eqn{(m,n)} dimensional matrix (or \code{NULL}). See the details below. 
#'
#' @return \code{\link{stsp}} object, which represents the generated rational matrix 
